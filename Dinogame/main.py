...
# Load images
track = pygame.transform.scale(pygame.image.load(r"D:\PYTHON\Pygame\Dino game\base.png"), [screen_width, 50])
cloud = pygame.image.load(os.path.join("Images and sounds", "Other", "Cloud.png"))
...
background_not_final = pygame.image.load(r"D:\PYTHON\Pygame\Dino game\background-day.png")
background = pygame.transform.scale(background_not_final, [screen_width, screen_height])
welcome_back_not_final = pygame.image.load(r"D:\PYTHON\Pygame\Dino game\edited back.png")
welcome_back = pygame.transform.scale(welcome_back_not_final, [screen_width, screen_height])
...
def baseBlit(gameWindow, track, track_x, track_y):
    gameWindow.blit(track, [track_x, track_y])
    gameWindow.blit(track, [track_x + 900, track_y])

def cloudBlit(cloud, cloud1_x, cloud1_y, cloud2_x, cloud2_y, cloud3_x, cloud3_y):
    gameWindow.blit(cloud, [cloud1_x, cloud1_y])
    gameWindow.blit(cloud, [cloud2_x, cloud2_y])
    gameWindow.blit(cloud, [cloud3_x, cloud3_y])
...
def welcome():
    ...
    while not exit_game:
        if game_over:
            ...
            control_button_rect = pygame.Rect(10, 10, game_controls.get_width(), game_controls.get_height())
            gameWindow.blit(game_controls, [10, 10])
            text_screen(55, "RBIG", white, gameWindow, 700, 40)
            text_screen(55, "Press SPACE BAR to play", red, gameWindow, 230, 200)
...
def gameloop():
    ...
    track_velocity = 0
    track_x, track_y = 0, screen_height - track.get_height()

    cloud_velocity = 0
    cloud1_x, cloud1_y = 50, 25
    cloud2_x, cloud2_y = 300, 30
    cloud3_x, cloud3_y = 700, 35
    large_cactus1_x, large_cactus1_y = 800, 460
    large_cactus2_x, large_cactus2_y = 1700, 460
    large_cactus3_x, large_cactus3_y = 2200, 460
    small_cactus1_x, small_cactus1_y = 2700, 480
    small_cactus2_x, small_cactus2_y = 3050, 480
    small_cactus3_x, small_cactus3_y = 3500, 480
    cactus_velocity = 0
    ...
    while not exit_game:
        if game_over:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RSHIFT:
                        track_velocity = -base_init_vel
                        cloud_velocity = -cloud_init_vel
                        cactus_velocity = -cactus_init_vel
                        dino_change_vel = 1
            ...
            if cloud1_x + cloud.get_width() < 0:
                cloud1_x = screen_width + 73.733
            if cloud2_x + cloud.get_width() < 0:
                cloud2_x = screen_width + 377.737
            if cloud3_x + cloud.get_width() < 0:
            ...
            baseBlit(gameWindow, track, track_x, track_y)
            cloudBlit(cloud, cloud1_x, cloud1_y, cloud2_x, cloud2_y, cloud3_x, cloud3_y)
